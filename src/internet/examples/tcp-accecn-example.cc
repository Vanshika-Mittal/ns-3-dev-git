...
{
  // Note: Modern ns-3 requires the leading '/' in Config paths
...
int main (int argc, char *argv[])
{
  ...
  // Enable modern ns-3 ECN flag
  Config::SetDefault ("ns3::RedQueueDisc::UseEcn", BooleanValue (true));
  Config::SetDefault ("ns3::TcpSocketBase::UseEcn", StringValue ("On"));
  
  // Now set the EcnMode to AccEcn
  ...
  int ret = system (dirToSave.c_str ()); // captured return to avoid warnings
  ret = system ((dirToSave + "/pcap/").c_str ());
  ret = system ((dirToSave + "/Traces/").c_str ());
